...
uint64_t ntohllFromBuffer(const uint8_t* buffer) {
  ...
  memcpy(&resultUpper, buffer, 4);
  memcpy(&resultLower, buffer + 4, 4);

  uint64_t result = ntohl(resultUpper);
  result = result << sizeof(uint32_t) * 8;
  result |= ntohl(resultLower);
  return result;
}

void htonllToBuffer(uint64_t value, uint8_t* buffer) {
  htonlToBuffer(value >> 32, buffer);
...
#if !C98 && !defined(GOOGLE_GLIBCXX_VERSION)
...
void (*g_diagnostic_callback)(const char* diagnostic_message) =
...
void SendDashLog(const char* file, uint32_t line, const char* message,
  ...
  if (g_diagnostic_callback) {
    ...
    snprintf(line_buffer, sizeof(line_buffer), "%u", line);
    std::string log_message = std::string("{\n\t'Message':'") + message +
        "',\n\t'File':'" + file + "',\n" +
        "\t'Line':" + line_buffer + ",\n" +
        "\t'Reason':'" + reason + "'";
    if (extra_fields) {
    ...
    log_message += "\n}";
    g_diagnostic_callback(log_message.c_str());
  }
}
...
std::string DumpMemory(const uint8_t* memory, size_t length) {
  ...
  size_t count = 0;
  do {
    snprintf(buffer, sizeof(buffer), " 0x%02x", memory[count]);
    ++count;
    result += buffer;
    if (count == length) {
      break;
...
size_t wvcrc32(uint8_t* begin, int count) {
  return wvrunningcrc32(begin, count, INIT_CRC32);
}

// The following table was copied from the Widevine code and is has been
// in use as is for many years.
// TODO(justsomeguy) See if we can remove the table below and just use
// an algorithm.
size_t wvrunningcrc32(uint8_t* buffer, int count, uint32_t crc) {
  ...
  /* Calculate the CRC */
  while (count > 0) {
    crc = (crc << 8) ^ CRC32[(crc >> 24) ^ ((uint32_t) * buffer) ];
    buffer++;
    count--;
  }
